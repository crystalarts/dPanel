...
const createValheimServer = async (config) => {
  const connection = db;

  try {
    const { ownerToken, memory, cpus, ports, duration } = config;

    const name = await generateUniqueName("valheim");
    ...
    await docker.pull("lloesche/valheim-server", (err, stream) => {
      if (err) throw err;
    ...
    console.error("Error creating Valheim server:", error);
  }
};

module.exports = { createValheimServer };
