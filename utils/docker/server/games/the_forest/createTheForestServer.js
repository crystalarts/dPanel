...
const createTheForestServer = async (config) => {
  const connection = db;

  try {
    const { ownerToken, memory, cpus, ports, duration } = config;

    const name = await generateUniqueName("theforest");
    ...
    await docker.pull("sergeymikheev/theforest", (err, stream) => {
      if (err) throw err;
      docker.modem.followProgress(stream, (event) => console.log(event));
    });

    const container = await docker.createContainer({
      Image: "sergeymikheev/theforest",
      name: name,
      Tty: true,
      ExposedPorts: {
        [`${ports.tcp}/tcp`]: {},
    ...
    console.error("Error creating The Forest server:", error);
  }
};

module.exports = { createTheForestServer };
