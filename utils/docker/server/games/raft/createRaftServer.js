...
const createRaftServer = async (config) => {
  const connection = db;

  try {
    const { ownerToken, memory, cpus, ports, duration } = config;

    const name = await generateUniqueName("raft");
    ...
    await docker.pull("raft/raft-server", (err, stream) => {
      if (err) throw err;
      docker.modem.followProgress(stream, (event) => console.log(event));
    });

    const container = await docker.createContainer({
      Image: "raft/raft-server",
      name: name,
      Tty: true,
      ExposedPorts: {
        [`${ports.tcp}/tcp`]: {},
    ...
    console.error("Error creating Raft server:", error);
  } finally {
  }
};

module.exports = { createRaftServer };
