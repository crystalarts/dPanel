...
const createRustServer = async (config) => {
  const connection = db;

  try {
    const { ownerToken, memory, cpus, ports, duration } = config;

    const name = await generateUniqueName("rust");
    ...
    await docker.pull("rustserver/rustserver", (err, stream) => {
      if (err) throw err;
      docker.modem.followProgress(stream, (event) => console.log(event));
    });

    const container = await docker.createContainer({
      Image: "rustserver/rustserver",
      name: name,
      Tty: true,
      ExposedPorts: {
        [`${ports.tcp}/tcp`]: {},
    ...
    console.error("Error creating Rust server:", error);
  }
};

module.exports = { createRustServer };
