...
router.post('/_api/v1/keygenerator/create', async (req, res) => {
    const { servers, nodes, allocations, dbhost, users, mounts, dbservers, whitelistIpv4, description } = req.body;
    ...
    const query = `
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `;
    const values = [apiKey, description, lastUsed, created, createdBy, servers, nodes, allocations, users, dbhost, dbservers, mounts];
